...
# save annotation for future reference
...
comparison <- get.comparisons(diffmeth)[1] # B DMSO vs. NB DMSO
...
tab.genes <- get.table(diffmeth.genes,comparison, region.type="genes",return.data.frame=T)
...
tab.promoters <- get.table(diffmeth.promoters,comparison, region.type="promoters",return.data.frame=T)
...
tab.cpgislands <- get.table(diffmeth.cpgislands,comparison, region.type="cpgislands",return.data.frame=T)
...
# the rownames of the .order dataframe refers to the row numbers of top (combinedRanke) differential loci in the annotated.tab.sites dataframe
